      ...
      <v-flex xs12>
        <Modal height="300px" width="500px">
          ...
          <template slot="actions">
            <v-spacer></v-spacer>
            <v-btn color="pink" flat @click="setModal(false)">Close</v-btn>
          </template>
        </Modal>
        <v-card>
          <v-toolbar color="pink accent-1" dark>
            <v-icon size="18px">fa fa-list-alt</v-icon>
            <v-toolbar-title>List TODO</v-toolbar-title>
            <v-spacer></v-spacer>
                ...
                <v-list-tile :key="task.id" avatar>
                  ...
                  <v-list-tile-content>
                    ...
                    <v-list-tile-sub-title>
                      {{ 'Deadline: ' + formatDate(task.deadline) + ' | '+dayleft(task.deadline) }}
                    </v-list-tile-sub-title>
                  </v-list-tile-content>
          ...
          <v-list three-line>
            ...
            <v-slide-x-transition group tag="v-list">
              <template v-for="(task, i) in completedTasks">
                ...
                <v-list-tile :key="task.id" avatar>
                  <v-list-tile-avatar>
                    <v-icon class="success white--text">fa fa-calendar-check-o</v-icon>
                  </v-list-tile-avatar>
                  ...
                  <v-list-tile-content>
                    ...
                    <v-list-tile-sub-title>
                      {{ 'Selesai: '+formatDate(task.completed)+' | dibuat: '+task.created }}
                    </v-list-tile-sub-title>
                  </v-list-tile-content>
